...
function generateSimilarAiQuestions(lessonId: string, lessonTitle: string, baseQuestions: QuizQuestion[]): QuizQuestion[] {
  ...
  const g9Raw = GRADE_9_AI_PRACTICE_DATA[lessonId] || GRADE_9_AI_PRACTICE_DATA[id];
  const g9Direct = Array.isArray(g9Raw) ? g9Raw : (g9Raw as any)?.quizQuestions;
  if (g9Direct && g9Direct.length > 0) {
    return g9Direct.map((q: any, idx: number) => ({
      ...q,
      id: `${q.id}-${ts}-${idx}`,
      isAiGenerated: true,
    }));
  ...
  const directData = Array.isArray(directDataRaw) ? directDataRaw : (directDataRaw as any)?.quizQuestions || [];
  if (directData && directData.length > 0) {
    return directData.map((q: any, idx: number) => ({
      ...q,
      id: `${q.id}-${ts}-${idx}`,
      isAiGenerated: true,
    }));
  ...
  for (const [key, questions] of Object.entries(allAiDatasets)) {
    ...
    if (cleanKey === cleanId || cleanId.includes(cleanKey) || cleanKey.includes(cleanId)) {
      const qList = Array.isArray(questions) ? questions : (questions as any)?.quizQuestions || [];
  ...
  const aiTfList: TrueFalseQuestion[] = useMemo(() => {
    return !activeExam
      ? (GRADE_12_AI_PRACTICE_DATA[lessonId || ""]?.trueFalseQuestions ||
         ...
         (GRADE_7_AI_PRACTICE_DATA[lessonId || ""] as any)?.trueFalseQuestions ||
         (GRADE_6_AI_PRACTICE_DATA[lessonId || ""] as any)?.trueFalseQuestions ||
         [])
      : [];
  }, [activeExam, lessonId]);
  ...
  const aiSaList: ShortAnswerQuestion[] = useMemo(() => {
    return !activeExam
      ? (GRADE_12_AI_PRACTICE_DATA[lessonId || ""]?.shortAnswerQuestions ||
         ...
         (GRADE_7_AI_PRACTICE_DATA[lessonId || ""] as any)?.shortAnswerQuestions ||
         (GRADE_6_AI_PRACTICE_DATA[lessonId || ""] as any)?.shortAnswerQuestions ||
         [])
      : [];
  }, [activeExam, lessonId]);
